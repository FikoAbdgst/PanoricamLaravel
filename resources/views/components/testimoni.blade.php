...
<body>
    <div class="hero-bg min-h-screen">
        ...
        <div class="text-center header-section bg-[#FEF3E2] py-20 relative" data-aos="fade-up">
            ...
            <p class="text-gray-600 text-xl max-w-4xl mx-auto px-6 leading-relaxed" data-aos="fade-up">
                ...
                ini dan punya pengalaman seru yang nggak terlupakan!
            </p>
    ...
    <div id="testimoniModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="text-2xl font-bold text-gray-800">Testimoni Lengkap</h2>
                <button class="modal-close" onclick="closeModal('testimoniModal')">×</button>
            </div>
            <div class="text-center">
                <div class="modal-icon">
                    <i class="fas fa-quote-left text-white text-3xl"></i>
                </div>
                <h3 id="testimoniName" class="text-xl font-semibold text-gray-800 mb-4">-</h3>
                <p id="testimoniMessage" class="text-gray-600 mb-2 leading-relaxed"></p>
                <div class="flex justify-center items-center my-4">
                    <div
                        class="flex items-center bg-gradient-to-r from-yellow-50 to-orange-50 rounded-full px-4 py-2 border border-yellow-200 w-fit">
                        <div id="testimoniStars" class="star-rating mr-3"></div>
                        <span id="testimoniRating" class="text-sm font-semibold text-gray-700">-</span>
                    </div>
                </div>
                <p id="testimoniDate" class="text-sm text-gray-500 font-medium"></p>
            </div>
            <div class="modal-buttons">
                <button class="btn btn-secondary" onclick="closeModal('testimoniModal')">Tutup</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function displayMarqueeTestimonis(testimonis) {
            ...
            container.innerHTML = duplicatedTestimonis.map((testimoni, index) => createTestimoniCard(testimoni, index))
                .join('');
        }

        function createTestimoniCard(testimoni, index) {
            ...
            return `
                <div class="testimonial-card card-gradient rounded-2xl py-7 px-10 shadow-xl mx-3 relative" onclick="showTestimoniModal(${index})">
                    ...
                    <div class="mt-auto">
                        ...
                        ${testimoni.frame ? `
                                <div class="frame-info-badge" onclick="event.stopPropagation(); handleFrameClickSimple(${testimoni.frame.id})">
        ...
        function showTestimoniModal(index) {
            const testimoni = allTestimonis[index % allTestimonis.length];
            if (!testimoni) return;

            const modal = document.getElementById('testimoniModal');
            document.getElementById('testimoniName').textContent = testimoni.name;
            document.getElementById('testimoniMessage').textContent = testimoni.message;
            document.getElementById('testimoniStars').innerHTML = generateStars(testimoni.rating);
            document.getElementById('testimoniRating').textContent = `(${testimoni.rating}/5)`;
            document.getElementById('testimoniDate').textContent = new Date(testimoni.created_at).toLocaleDateString(
                'id-ID', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
            ...
            pauseMarquee(); // Pause marquee saat modal testimoni muncul
        }
        ...
        function showModal() {
            ...
            document.body.style.overflow = 'hidden';
            pauseMarquee(); // Pause marquee saat modal pembayaran muncul
        }
        ...
        function closeModal(modalId) {
            ...
            setTimeout(() => {
                ...
                resumeMarquee(); // Lanjutkan marquee saat modal ditutup
            }, 300);
        ...
        function handleFrameClickSimple(frameId) {
            ...
            if (testimoni && testimoni.frame && testimoni.frame.price > 0) {
                document.getElementById('framePrice').textContent = `Rp ${testimoni.frame.price.toLocaleString('id-ID')}`;
            } else {
        ...
        window.onclick = function(event) {
            ...
            if (event.target === document.getElementById('testimoniModal')) {
        ...
        function generateStars(rating) {
            let stars = '';
            for (let i = 1; i <= 5; i++) {
                stars += i <= rating ? '<i class="fas fa-star"></i>' : '<i class="far fa-star text-gray-300"></i>';
            }
            return stars;
        }
        ...
        function loadMockData() {
            const mockTestimonis = [{
                    id: 1,
                    name: "Sarah Johnson",
                    message: "PhotoBooth ini sangat keren! Hasil fotonya berkualitas tinggi dan framenya beragam. Sangat puas dengan pelayanannya! Pengalaman yang luar biasa dan pasti akan kembali lagi untuk acara berikutnya!",
                    rating: 5,
                ...
                },
                {
                    id: 5,
                    name: "Maria Santos",
                    message: "Frame premium memang beda! Detail dan kualitasnya sangat memuaskan. Worth every penny! Saya sangat merekomendasikan untuk semua acara spesial Anda, hasilnya tidak akan mengecewakan!",
                    rating: 5,
